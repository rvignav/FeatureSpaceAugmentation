...
def train(net, loader, crit, optim, lr_adjuster=None, augmentor=None):
    ...
    for inputs, targets in loader:
        ...
        preds = nn_utils.get_argmax(output)
        accuracy = nn_utils.get_accuracy(preds, targets.data.cpu().numpy())
...
def test(net, tst_loader, criterion):
    ...
    for data in tst_loader:
        ...
        pred = nn_utils.get_argmax(output)
        test_acc += nn_utils.get_accuracy(pred, target.data.cpu().numpy())
